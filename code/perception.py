...
def perception_step(Rover):
    ...
    img_shape = Rover.img.shape
    source = np.float32([[14, 140], [301 ,140],[200, 96], [118, 96]])
    destination = np.float32([[img_shape[1]/2 - dst_size, img_shape[0] - bottom_offset],
    ...
    # To deal with pure black, I have to threshold the original image before thransforming.
    ...
    obstacles_x_rover, obstacles_y_rover = rover_coords(obstacles)
    rock_x_rover, rock_y_rover = rover_coords(rock)

    # 6) Convert rover-centric pixel values to world coordinates
    obstacle_x_world, obstacle_y_world = pix_to_world(obstacles_x_rover, obstacles_y_rover, 
